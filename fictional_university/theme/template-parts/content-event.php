<div class="event-summary">
  <a class="event-summary__date t-center" href="#">
    <span class="event-summary__month"><?php 
      $event_date = new DateTime(get_field('event_date', false, false)); 
      // Loome objekti, mis v6tab ACF-ist valitud custom fieldi (event date)
      echo $event_date->format('M') 
      // ning m22rame sellele omalt poolt formaadi (v2ljastab vaid kuu lyhinime).
      ?></span>
    <span class="event-summary__day"><?php echo $event_date->format('d') ?></span> 
    <!-- v2ljastame objekti ning muudame selle formaati -->
  </a>
  <div class="event-summary__content">
    <h5 class="event-summary__title headline headline--tiny"><a href="<?php echo get_permalink(); ?>"><?php the_title(); ?></a></h5>
    <p><?php
      if(has_excerpt()) {
        echo get_the_excerpt();
      } else {
        echo wp_trim_words(get_the_content(), 18);
      }
    ?> 
    <a href="<?php echo get_permalink(); ?>" class="nu gray">Learn more</a></p>
  </div>
</div>